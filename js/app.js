...
class Player {
  ...
  setPlayerPosition() {
    ...
    //avoid players from being adjacent to each other
    const adjacents = [cell - 1, cell - 10, cell + 1, cell + 10];

    // fill adjacent cells to the player with values to indicate not empty
    // dissallowing the cell from being occupied by another player
  ...
  // create range of movements which are valid
...
// instantiate players
const player1 = new Player('player1', 'master-f.png');
const player2 = new Player('player2', 'horn.png');

// instantiate the objects
const sword = new Weapons('sword', 'sword.png', 18);
const gun = new Weapons('gun', 'gun.png', 15);
const axe = new Weapons('axe', 'axe.png', 14);
...
function searchWeapon(searchWeaponFrom, searchWeaponTo) {
  ...
  for (var i = 0; i <= movedToBoxes.length; i++) {
    ...
    } else if (movedToBox.hasClass('axe')) {
      newWeapon = 'axe';
